...
import ru.letu.ui.screens.AccountProfileScreen
...
class AccountProfileTest : AllureTestCase() {
    ...
    fun test() {
        init {
            ...
            step("11-Проверка загрузки профиля") {
                AccountProfileScreen {
                    checkConsistency()
                    checkConsistencyProfileInfo()
                    checkConsistencyOrderWidget()
                    checkConsistencyCouponsWidget()
                    checkConsistencyLoyaltyNplWidget()
                    checkConsistencyMenuLink()
                }
            }
        }
    }
